...
const UpdateUser = ({ user }: { user: User }) => {
  ...
  const [open, setOpen] = useState(false);          // Trạng thái bật/tắt modal cập nhật
  const [cover, setCover] = useState<any>(false);   // Ảnh cover mới từ Cloudinary (nếu có)

  // Dùng useActionState để xử lý form action bất đồng bộ
  const [state, formAction] = useActionState(updateProfile,{success:false,error:false});

  const router = useRouter();

  // Hàm đóng modal, nếu cập nhật thành công thì reload lại trang
  const handleClose = () => {
    setOpen(false);
        ...
        <div className="absolute w-screen h-screen top-0 left-0 bg-black bg-opacity-65 flex items-center justify-center z-50">
          <form action={(formData)=>formAction({formData,cover:cover?.secure_url || ""})} 
            className="p-12 bg-white rounded-lg shadow-md flex flex-col gap-2 w-full md:w-1/2 xl:w-1/3 relative">
                ...
                {/* WRAPPER - Thay đổi từ flex sang grid */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <input type="text" placeholder={user.description || "Dm Cuoc doi....."} className="ring-1 ring-gray-300 p-[13px] rounded-md text-sm" name="description"/>
                    </div>
                    {/* INPUT */}              
                    <div className="flex flex-col gap-2">
                      <label htmlFor="" className="text-xs text-gray-500">City</label>
                      <input type="text" placeholder={user.city || "Ho Chi Minh"} className="ring-1 ring-gray-300 p-[13px] rounded-md text-sm" name="city"/>
                    </div>
                    {/* INPUT */}
                    <div className="flex flex-col gap-2">
                      <label htmlFor="" className="text-xs text-gray-500">School</label>
                      <input type="text" placeholder={user.school || "HSU"} className="ring-1 ring-gray-300 p-[13px] rounded-md text-sm"  name="school"/>
                    </div>
                    {/* INPUT */}
                    <div className="flex flex-col gap-2">
                      <label htmlFor="" className="text-xs text-gray-500">Work</label>
                      <input type="text" placeholder={user.work || "......."} className="ring-1 ring-gray-300 p-[13px] rounded-md text-sm" name="work"/>
                    </div>
                    {/* INPUT */}
                    <div className="flex flex-col gap-2">
                      <label htmlFor="" className="text-xs text-gray-500">Website</label>
                      <input type="text" placeholder={user.website || "TienNT.git"} className="ring-1 ring-gray-300 p-[13px] rounded-md text-sm" name="website"/>
                    </div>
                </div>
